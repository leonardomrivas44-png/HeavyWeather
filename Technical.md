...
The short explanation of key systems:
  After clicking 'start', a list of rooms is generated as structs alongside their connections and special rooms, moving between rooms changes which struct is accessed.
  Perks are created as structs holding stat changes, requirements, and abilities. Updates on perk availability is checked constantly, when one is avaible, it's marked until it is unlocked.
  Items are also structs including stat changes and abilities. Dropped items use a generic 'item' object, that references the items sprite and ID for retrieval.
  Enemy formations are picked from a pool of 2 difficulties depending on which stage of the floor you're on. Each enemies location is store in regards to the center of the room; when loaded, it simply calculates which way to orient the formation.
...
Room Generation:
  ...
  Using this struct, the generator places the starting room at 0,0 with ID of 0 in the array and starts the creation loop.
    This loop holds an algorithm which ends after creating a random amount of rooms, dependent on which part the player is on.
    Using a random room from the array as a base, the algorithm picks a random direction and check for population:
      if the prospective room is populated, the algorithm continues without creating new room, not incrementing the loop
  ...
  After the array is populated, leftover room artifacts are cleared and certain rooms are picked for special properties.
    Room ID 0 (starting room, center of map), is set at the START room, removing any enemies in it.
...
![Part 1 map example](Screenshots-and-Images/image.png)
